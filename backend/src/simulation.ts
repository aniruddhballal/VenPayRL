...
export function createStateFromScenario(scenario: ScenarioConfig): SimState {
  return {
    ...
    cash: scenario.cash,
    totalReward: 0,
    invoices: scenario.invoices.map(inv => ({
      ...inv,
      id: uuid(),
      paid: false,
      delayed: 0,
...
export function stepSimulation(state: SimState, actions: AgentAction[]): SimState {
  ...
  for (const inv of next.invoices) {
    ...
    if (action?.type === 'full') {
      // Constraint: never pay more than available cash
      if (next.cash < cost) {
        // Can't afford — force delay
        inv.delayed += 1
      ...
      // Constraint: only partial pay if we have something to pay
      if (partial <= 0) {
        inv.delayed += 1
        const penalty = inv.amount * inv.penaltyRate
        stepReward -= penalty
